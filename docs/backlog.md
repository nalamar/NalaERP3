...
- [ ] E.5 (E.5.1-E.5.2 abgeschlossen) E-Rechnung Eingang (mind. Parsen) — wip, in Subtasks zerlegt (§6.3, analog zu Epic D/E.1/E.2/E.3/E.4)
  ...
  - [ ] E.5.3 UBL-Eingangsparser (DB-los) + Formaterkennung über den Wurzelelement-Namensraum, auf dasselbe Zielmodell + Unit-Tests — todo.
  - [ ] E.5.4 ZUGFeRD-PDF: Abhängigkeit `pdfcpu` aufnehmen und begründen, eingebetteten Anhang extrahieren, an den CII-Parser übergeben + Tests — todo. Beim Umsetzen die exakte API der gepinnten Version erneut gegen den Quellcode prüfen (die Signatur hat sich zwischen Versionen geändert; die in E.5.1 verifizierte Form stammt aus `master`).
  - [ ] E.5.5 (letzte Subtask von Task E.5) HTTP-Endpunkt `POST /invoices-in/parse-e-invoice` inkl. Lieferanten-Zuordnungsvorschlag, Größenbegrenzung und Entitäten-Nachweis + End-to-End-Integrationstest — todo.
- [ ] E.6 Übernahme einer geparsten Eingangs-E-Rechnung nach `invoices_in` (Lieferantenbestätigung, Bestellzuordnung) — todo. Entstanden aus ADR 0023/E.5.1: bewusst NICHT Teil von E.5, weil `invoices_in.supplier_id` ein Pflicht-Fremdschlüssel auf `contacts` ist und eine automatische Anlage Stammdaten aus einer von außen zugestellten Datei erzeugen bzw. einen geratenen Lieferanten zuordnen würde. E.5 liefert die geparsten Daten und einen Zuordnungsvorschlag; die Übernahme bestätigt ein Mensch.
