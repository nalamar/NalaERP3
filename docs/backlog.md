...
- [ ] E.4 (E.4.1-E.4.3.1 abgeschlossen) E-Rechnung Ausgang (XRechnung, ZUGFeRD 2.x) — wip, in Subtasks zerlegt (§6.3, analog zu Epic D/E.1/E.2/E.3)
  ...
  - [ ] E.4.3 (E.4.3.1 abgeschlossen) Anwendungscode: CII-XML-Builder, DB-Abfrage/Mapping, HTTP-Wiring (XRechnung + ZUGFeRD-PDF-Embedding), Tests — wip. Größenschätzung bestätigt sich (verschachtelte CII-Struktur mit vier Ebenen, zwei Endpunkte, PDF-Embedding-Logik, mehrere Testfälle inkl. Negativfälle) und überschreitet §6.3 — wie in ADR 0022 "Konsequenzen" vorgezeichnet in vier Micro-Subtasks zerlegt (analog D.3.3/E.2.3/E.3.3): E.4.3.1 (reiner CII-Serialisierer, DB-los, vollständig eigenständig verifizierbar), E.4.3.2 (DB-Abfrage/Mapping inkl. Käuferadress-Auflösung und Steuerkategorie-Ableitung), E.4.3.3 (HTTP-Wiring XRechnung), E.4.3.4 (ZUGFeRD-PDF-Embedding, letzte Subtask von Task E.4).
    ...
    - [ ] E.4.3.2 DB-Abfrage/Mapping: `invoices_out`→`CIIInvoice` inkl. Käuferadress-Auflösung aus `contact_addresses` (`billing` > `is_primary` > irgendeine, sonst Fehler), Steuerkategorie-Ableitung aus `tax_codes` (`rate>0`→`S`, `rate=0 AND reverse_charge`→`AE`, sonst `E`), Steueraufschlüsselung je Steuersatz-Gruppe, Typ-Code-Zuordnung, Status-Guard (`booked`/`paid`, `draft`/`storniert` abgelehnt) + Integrationstests — todo.
