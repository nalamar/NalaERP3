...
class _ProjectsPageState extends State<ProjectsPage> {
        ...
        if (zip != null) {
          try {
            ...
            if (mounted) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Assets-Upload fehlgeschlagen: $e')));
            }
          }
        }
      }
  ...
  Future<void> _analyzeLogikal() async {
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('Analyse fehlgeschlagen: $e')));
    }
  }
  ...
  Future<void> _addProject() async {
    ...
    try {
      ...
      Navigator.of(context)
          .push(MaterialPageRoute(
              builder: (_) =>
                  ProjectDetailPage(api: widget.api, project: created)))
          .then((_) => _load());
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
...
class ProjectDetailPage extends StatefulWidget {
  const ProjectDetailPage(
      {super.key, required this.api, required this.project});
  final ApiClient api;
...
class _ProjectDetailPageState extends State<ProjectDetailPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final list =
          await widget.api.listProjectPhases(widget.project['id'] as String);
      setState(() {
        _phases = list;
      });
    } catch (e) {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(children: [
          _buildStructureTab(),
          ImportLogTab(
              api: widget.api, projectId: widget.project['id'] as String),
        ]),
      ),
    );
  }
      ...
      child: SingleChildScrollView(
        child:
          ...
          FilledButton.icon(
              ...
              label: const Text('Rechnung schreiben (bald)')),
...
class _VariantTileState extends State<_VariantTile> {
        ...
        if (_materials != null)
          Padding(
            ...
            child:
              ...
              const SizedBox(height: 6),
              ...List<Widget>.from(
                  ((_materials!['articles'] ?? []) as List).map((a0) {
                final a = a0 as Map<String, dynamic>;
                final materialLink =
                    ((a)['material_nummer'] ?? '').toString().isNotEmpty
                        ? '  •  verknüpft: ' + (a)['material_nummer']
                        : '';
                return ListTile(
                  dense: true,
                  leading: const Icon(Icons.extension_rounded),
                  title: Text(((a)['article_code'] ?? (a)['description'] ?? '')
                      .toString()),
                  subtitle: Text(
                      'Menge ${(a)['qty'] ?? 1} ${(a)['unit'] ?? ''}$materialLink'),
              ...
              const SizedBox(height: 6),
              ...List<Widget>.from(
                  ((_materials!['glass'] ?? []) as List).map((g0) {
                final g = g0 as Map<String, dynamic>;
                final materialLink =
                    ((g)['material_nummer'] ?? '').toString().isNotEmpty
                        ? '  •  verknüpft: ' + (g)['material_nummer']
                        : '';
                return ListTile(
                  dense: true,
                  leading: const Icon(Icons.window_rounded),
                  title: Text(((g)['configuration'] ?? (g)['description'] ?? '')
                      .toString()),
  ...
  Future<void> _unlink(Map<String, dynamic> it, String kind) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('Lösen fehlgeschlagen: $e')));
    } finally {
      if (mounted)
        setState(() {
          _saving = false;
  ...
  Future<void> _adoptMaterial(Map<String, dynamic> it, String kind) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Übernahme fehlgeschlagen: $e')));
    } finally {
      if (mounted)
        setState(() {
          _saving = false;
...
class _ImportTileState extends State<_ImportTile> {
  ...
  void _downloadCSV() {
    final url = widget.api.baseUrl +
        '/api/v1/projects/${widget.projectId}/imports/${widget.importRun['id']}/changes?format=csv';
    browser.downloadUrl(url, filename: 'import-${widget.importRun['id']}.csv');
  }

  void _downloadJSON() {
    final url = widget.api.baseUrl +
        '/api/v1/projects/${widget.projectId}/imports/${widget.importRun['id']}/changes';
  ...
  Future<void> _undo() async {
    ...
    try {
      final uri = Uri.parse(widget.api.baseUrl +
          '/api/v1/projects/${widget.projectId}/imports/${widget.importRun['id']}/undo');
      final resp = await http.post(uri);
      if (resp.statusCode != 200)
        throw Exception('Fehler: ${resp.statusCode} ${resp.body}');
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Rückgängig fehlgeschlagen: $e')));
    }
  }
}
          ...
          Flexible(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final subtitle = [c['email'], c['telefon']]
                    .whereType<String>()
                    .where((s) => s.isNotEmpty)
                    .join(' • ');
                return ListTile(
