...
class _ContactDetailScreenState extends State<ContactDetailScreen> {
  ...
  Future<void> _loadAll() async {
    ...
    try {
      ...
      setState(() { contact = c; addresses = a; persons = p; notes = n; tasks = t; documents = d; activity = h; });
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(_errorMessage(e, fallback: 'Laden fehlgeschlagen'))),
        );
      }
  ...
  Future<void> _editContact() async {
    ...
    final paymentTerms = TextEditingController(text: (c['zahlungsbedingungen'] ?? '').toString());
    final debtor = TextEditingController(text: (c['debitor_nr'] ?? '').toString());
    final creditor = TextEditingController(text: (c['kreditor_nr'] ?? '').toString());
    final taxCountry = TextEditingController(text: (c['steuer_land'] ?? 'DE').toString());
      ...
      content: SizedBox(
        ...
        child: SingleChildScrollView(
          child: Wrap(spacing: 12, runSpacing: 12, children: [
            SizedBox(width: 160, child: DropdownButtonFormField<String>(value: typ, items: [const DropdownMenuItem(value:'org',child:Text('Organisation')), const DropdownMenuItem(value:'person',child:Text('Person'))], onChanged: (v){ typ = v ?? 'org'; }, decoration: const InputDecoration(labelText: 'Typ'))),
            SizedBox(width: 200, child: DropdownButtonFormField<String>(value: rolle, items: [const DropdownMenuItem(value:'customer',child:Text('Kunde')), const DropdownMenuItem(value:'supplier',child:Text('Lieferant')), const DropdownMenuItem(value:'partner',child:Text('Partner')), const DropdownMenuItem(value:'both',child:Text('Kunde & Lieferant')), const DropdownMenuItem(value:'other',child:Text('Sonstige'))], onChanged: (v){ rolle = v ?? 'other'; }, decoration: const InputDecoration(labelText: 'Rolle'))),
            SizedBox(width: 180, child: DropdownButtonFormField<String>(value: status, items: [const DropdownMenuItem(value:'lead',child:Text('Interessent')), const DropdownMenuItem(value:'active',child:Text('Aktiv')), const DropdownMenuItem(value:'inactive',child:Text('Inaktiv')), const DropdownMenuItem(value:'blocked',child:Text('Gesperrt'))], onChanged: (v){ status = v ?? 'active'; }, decoration: const InputDecoration(labelText: 'Status'))),
            ...
            SizedBox(width: 220, child: TextFormField(controller: paymentTerms, decoration: const InputDecoration(labelText: 'Zahlungsbedingungen'))),
            SizedBox(width: 180, child: TextFormField(controller: debtor, decoration: const InputDecoration(labelText: 'Debitor-Nr.'))),
            SizedBox(width: 180, child: TextFormField(controller: creditor, decoration: const InputDecoration(labelText: 'Kreditor-Nr.'))),
            SizedBox(width: 120, child: TextFormField(controller: taxCountry, decoration: const InputDecoration(labelText: 'Steuerland'))),
        ...
        FilledButton.icon(onPressed: () async {
          try {
            final patch = {
              ...
              'steuer_land': taxCountry.text.trim().isEmpty ? 'DE' : taxCountry.text.trim().toUpperCase(),
  ...
  Future<void> _editAddress(Map<String,dynamic>? addr) async {
    ...
    await showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      actions: [
        if (!isNew) TextButton(onPressed: () async {
          try {
            await widget.api.deleteContactAddress(widget.id, (addr!['id'] as String));
          ...
          child: SingleChildScrollView(
            child: Wrap(spacing: 12, runSpacing: 12, children: [
              ...
              SizedBox(width: 180, child: DropdownButtonFormField<String>(
                value: role,
                items: const [
                  DropdownMenuItem(value:'management', child: Text('Geschäftsführung')),
                  DropdownMenuItem(value:'purchasing', child: Text('Einkauf')),
                  DropdownMenuItem(value:'sales', child: Text('Vertrieb')),
                  DropdownMenuItem(value:'accounting', child: Text('Buchhaltung')),
                  DropdownMenuItem(value:'project', child: Text('Projekt')),
                  DropdownMenuItem(value:'technical', child: Text('Technik')),
                  DropdownMenuItem(value:'other', child: Text('Sonstige')),
                ],
                onChanged: (val) => setModalState(() => role = val ?? 'other'),
              ...
              SizedBox(width: 180, child: DropdownButtonFormField<String>(
                ...
                items: const [
                  ...
                  DropdownMenuItem(value:'mobile', child: Text('Mobil')),
                ...
                decoration: const InputDecoration(labelText: 'Bevorzugter Kanal'),
              )),
              ...
              Row(children: [Checkbox(value: primary, onChanged: (val)=> setModalState(()=> primary = val ?? primary)), const Text('Primär')]),
            ]),
          ),
        ),
        actions: [
          if (!isNew) TextButton(onPressed: () async {
            try {
              await widget.api.deleteContactPerson(widget.id, (pers!['id'] as String));
              Navigator.of(ctx).pop();
              await _loadAll();
            } catch (e) {
              if (mounted) {
          ...
          FilledButton(onPressed: () async {
            try {
              ...
              if (isNew) {
                await widget.api.createContactPerson(widget.id, payload);
              } else {
                await widget.api.updateContactPerson(widget.id, (pers!['id'] as String), payload);
              }
              if (mounted) Navigator.of(ctx).pop();
              await _loadAll();
            } catch (e) {
  ...
  Future<void> _editNote(Map<String, dynamic>? note) async {
    ...
    final titel = TextEditingController(text: (note?['titel'] ?? '').toString());
    final inhalt = TextEditingController(text: (note?['inhalt'] ?? '').toString());
    await showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: SizedBox(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              TextFormField(controller: titel, decoration: const InputDecoration(labelText: 'Titel')),
              const SizedBox(height: 12),
              TextFormField(controller: inhalt, minLines: 4, maxLines: 8, decoration: const InputDecoration(labelText: 'Inhalt')),
            ],
          ),
        ),
      ),
      actions: [
        if (!isNew) TextButton(onPressed: () async {
          try {
            await widget.api.deleteContactNote(widget.id, (note!['id'] as String));
            if (mounted) Navigator.of(ctx).pop();
            await _loadAll();
          } catch (e) {
            if (mounted) {
        ...
        FilledButton(onPressed: () async {
          try {
            if (isNew) {
              await widget.api.createContactNote(widget.id, {'titel': titel.text.trim(), 'inhalt': inhalt.text.trim()});
            } else {
              await widget.api.updateContactNote(widget.id, (note!['id'] as String), {'titel': titel.text.trim(), 'inhalt': inhalt.text.trim()});
            }
            if (mounted) Navigator.of(ctx).pop();
            await _loadAll();
          } catch (e) {
  ...
  Future<void> _editTask(Map<String, dynamic>? task) async {
    ...
    final titel = TextEditingController(text: (task?['titel'] ?? '').toString());
    final beschreibung = TextEditingController(text: (task?['beschreibung'] ?? '').toString());
    String status = (task?['status'] ?? 'open').toString();
    DateTime? dueDate = DateTime.tryParse((task?['faellig_am'] ?? '').toString())?.toLocal();
    await showDialog(context: context, builder: (ctx) => StatefulBuilder(
      builder: (ctx, setModalState) => AlertDialog(
        title: Text(isNew ? 'Aufgabe hinzufügen' : 'Aufgabe bearbeiten'),
        content: SizedBox(
          width: 560,
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(value: 'canceled', child: Text('Abgebrochen')),
                  ],
                  onChanged: (v) => setModalState(() => status = v ?? 'open'),
                  decoration: const InputDecoration(labelText: 'Status'),
                ),
                const SizedBox(height: 12),
                TextFormField(controller: beschreibung, minLines: 3, maxLines: 6, decoration: const InputDecoration(labelText: 'Beschreibung')),
                const SizedBox(height: 12),
                Row(
                  children: [
                    Expanded(
                      child: Text('Fällig am: ${dueDate == null ? '—' : _formatDateTime(dueDate.toUtc().toIso8601String())}'),
                    ),
                    TextButton.icon(
                      onPressed: () async {
        ...
        actions: [
          if (!isNew) TextButton(onPressed: () async {
            try {
              await widget.api.deleteContactTask(widget.id, (task!['id'] as String));
              if (mounted) Navigator.of(ctx).pop();
              await _loadAll();
            } catch (e) {
              if (mounted) {
            ...
            try {
              final payload = {
                ...
                'faellig_am': dueDate == null
                    ? ''
                    : DateTime(dueDate!.year, dueDate!.month, dueDate!.day).toUtc().toIso8601String(),
              };
              if (isNew) {
                await widget.api.createContactTask(widget.id, payload);
              } else {
                await widget.api.updateContactTask(widget.id, (task!['id'] as String), payload);
              }
              if (mounted) Navigator.of(ctx).pop();
              await _loadAll();
            } catch (e) {
  ...
  Future<void> _uploadDocument() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(_errorMessage(e, fallback: 'Dokument-Upload fehlgeschlagen'))),
        );
      }
    }
  }
      ...
      floatingActionButton: canWrite
          ? Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                FloatingActionButton.small(heroTag: 'note_add', onPressed: ()=> _editNote(null), child: const Icon(Icons.note_add_outlined)),
                const SizedBox(height: 8),
                FloatingActionButton.small(heroTag: 'task_add', onPressed: ()=> _editTask(null), child: const Icon(Icons.task_alt_outlined)),
                const SizedBox(height: 8),
                FloatingActionButton.small(heroTag: 'doc_add', onPressed: _uploadDocument, child: const Icon(Icons.upload_file_outlined)),
              ],
            )
          : null,
      body: loading && c==null
        ...
        : Padding(
            ...
            child: Column(
              ...
              children: [
                if (c != null) ...[
                  Text('Typ: ${_typeLabel((c['typ'] ?? '').toString())}  •  Rolle: ${_roleLabel((c['rolle'] ?? '').toString())}  •  Status: ${_statusLabel((c['status'] ?? 'active').toString())}  •  Währung: ${c['waehrung'] ?? 'EUR'}'),
                  const SizedBox(height: 8),
                  Text('E-Mail: ${c['email'] ?? ''}  •  Telefon: ${c['telefon'] ?? ''}'),
                  const SizedBox(height: 8),
                  Text('Zahlungsbedingungen: ${(c['zahlungsbedingungen'] ?? '').toString().isEmpty ? '—' : c['zahlungsbedingungen']}  •  Debitor: ${(c['debitor_nr'] ?? '').toString().isEmpty ? '—' : c['debitor_nr']}  •  Kreditor: ${(c['kreditor_nr'] ?? '').toString().isEmpty ? '—' : c['kreditor_nr']}'),
                  const SizedBox(height: 8),
                  Text('Steuerland: ${(c['steuer_land'] ?? 'DE')}  •  Steuerbefreit: ${(c['steuerbefreit'] ?? false) == true ? 'Ja' : 'Nein'}'),
                  const Divider(),
                ],
                Expanded(
                  child: Row(
                    children: [
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (ctx, i) {
                                  ...
                                  return ListTile(
                                    ...
                                    trailing: canWrite
                                        ? IconButton(icon: const Icon(Icons.edit), onPressed: ()=> _editAddress(a))
                                        : null,
                                  );
                                },
                              ),
                            ...
                            Expanded(
                              child: ListView.builder(
                                ...
                                itemBuilder: (ctx, i) {
                                  ...
                                  final role = (p['rolle'] ?? '').toString().trim();
                                  final channel = (p['bevorzugter_kanal'] ?? '').toString().trim();
                                  return ListTile(
                                    dense: true,
                                    title: Text(name.isEmpty? (p['email']??'').toString() : name),
                                    subtitle: Text('${p['position'] ?? ''}${role.isNotEmpty ? '  •  ${_personRoleLabel(role)}' : ''}${channel.isNotEmpty ? '  •  ${_channelLabel(channel)}' : ''}  •  ${p['email'] ?? ''}  •  ${p['telefon'] ?? ''}${p['is_primary']==true ? ' • Primär' : ''}'),
                                    trailing: canWrite
                                        ? IconButton(icon: const Icon(Icons.edit), onPressed: ()=> _editPerson(p))
                                        : null,
                                  );
                                },
                              ),
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Notizen', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 6),
                            Expanded(
                              child: ListView.builder(
                                ...
                                itemBuilder: (ctx, i) {
                                  ...
                                  final title = (n['titel'] ?? '').toString().trim();
                                  final body = (n['inhalt'] ?? '').toString().trim();
                                  return ListTile(
                                    dense: true,
                                    ...
                                    trailing: canWrite
                                        ? IconButton(icon: const Icon(Icons.edit), onPressed: ()=> _editNote(n))
                                        : null,
                                  );
                                },
                              ),
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Aufgaben', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 6),
                            Expanded(
                              child: ListView.builder(
                                ...
                                itemBuilder: (ctx, i) {
                                  ...
                                  final due = _formatDateTime((t['faellig_am'] ?? '').toString());
                                  final done = _formatDateTime((t['erledigt_am'] ?? '').toString());
                                  final description = (t['beschreibung'] ?? '').toString().trim();
                                  return ListTile(
                                    dense: true,
                                    title: Text((t['titel'] ?? '').toString().trim().isEmpty ? 'Ohne Titel' : (t['titel'] ?? '').toString()),
                                    subtitle: Text(
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Dokumente', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 6),
                            Expanded(
                              child: ListView.builder(
                                ...
                                itemBuilder: (ctx, i) {
                                  ...
                                  return ListTile(
                                    ...
                                    title: Text((d['filename'] ?? d['document_id']).toString()),
                                    subtitle: Text('${d['content_type'] ?? ''}  •  ${d['length'] ?? 0} B'),
                                    trailing: IconButton(
                                      icon: const Icon(Icons.download),
                                      onPressed: () => widget.api.downloadDocument(
                                        (d['document_id'] ?? '').toString(),
                                        filename: (d['filename'] ?? '').toString().isEmpty ? null : (d['filename'] ?? '').toString(),
                                      ),
                                    ),
                                  );
                                },
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Verlauf', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 6),
                            Expanded(
                              child: activity.isEmpty
                                  ...
                                  : ListView.builder(
                                      ...
                                      itemBuilder: (ctx, i) {
                                        ...
                                        final description = (item['beschreibung'] ?? '').toString().trim();
                                        final source = _activitySourceLabel((item['quelle'] ?? '').toString());
                                        final when = _formatDateTime((item['zeitpunkt'] ?? '').toString());
                                        return ListTile(
                                          dense: true,
                                          title: Text(title),
                                          subtitle: Text(
