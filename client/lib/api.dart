...
class ApiClient {
  ...
  Future<Map<String, dynamic>> uploadPdfImage(
    ...
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
      throw Exception(
          'Fehler: ${resp.statusCode} ${utf8.decode(resp.bodyBytes)}');
    }
  ...
  Future<Map<String, dynamic>> createProject(Map<String, dynamic> body) async {
    final r = await http.post(_u('/api/v1/projects/'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(body));
    if (r.statusCode != 201) {
      throw Exception(utf8.decode(r.bodyBytes));
    }
    return jsonDecode(utf8.decode(r.bodyBytes));
  }
  ...
  Future<Map<String, dynamic>> importLogikalProject(
    ...
    if (r.statusCode != 201) {
      throw Exception(
          'Import fehlgeschlagen: ${r.statusCode} ${utf8.decode(r.bodyBytes)}');
    }
    return jsonDecode(utf8.decode(r.bodyBytes)) as Map<String, dynamic>;
  }
  ...
  Future<Map<String, dynamic>> analyzeLogikalProject(
    ...
    try {
      ...
      if (r.statusCode != 200) {
        throw Exception(
            'Analyse fehlgeschlagen: ${r.statusCode} ${utf8.decode(r.bodyBytes)}');
      }
      return jsonDecode(utf8.decode(r.bodyBytes)) as Map<String, dynamic>;
    } catch (_) {
      final headers = <String, String>{
  ...
  Future<Map<String, dynamic>> createPhase(
    ...
    final r = await http.post(_u('/api/v1/projects/$projectId/phases'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(body));
    if (r.statusCode != 201)
      throw Exception('Fehler: ${r.statusCode} ${utf8.decode(r.bodyBytes)}');
    return jsonDecode(utf8.decode(r.bodyBytes));
  ...
  Future<void> deletePhase(String projectId, String phaseId) async {
    final r =
        await http.delete(_u('/api/v1/projects/$projectId/phases/$phaseId'));
    if (r.statusCode != 204)
      throw Exception('Fehler: ${r.statusCode} ${utf8.decode(r.bodyBytes)}');
  }
  ...
  Future<void> uploadProjectAssets(
    ...
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
      throw Exception(
          'Assets-Upload fehlgeschlagen: ${resp.statusCode} ${utf8.decode(resp.bodyBytes)}');
    }
  }
  ...
  Future<Map<String, dynamic>> updatePurchaseOrder(
    ...
    final r = await http.patch(_u('/api/v1/purchase-orders/$id'),
  ...
  Future<Map<String, dynamic>> createPurchaseOrderItem(
    ...
    final r = await http.post(_u('/api/v1/purchase-orders/$orderId/items'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(body));
  ...
  Future<void> deletePurchaseOrderItem(String orderId, String itemId) async {
    final r =
        await http.delete(_u('/api/v1/purchase-orders/$orderId/items/$itemId'));
  ...
  Future<Map<String, dynamic>> createMaterial(Map<String, dynamic> body) async {
    final r = await http.post(_u('/api/v1/materials/'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(body));
    if (r.statusCode != 201) throw Exception(utf8.decode(r.bodyBytes));
    return jsonDecode(utf8.decode(r.bodyBytes));
  }
  ...
  Future<Map<String, dynamic>> updateMaterial(
      String id, Map<String, dynamic> patch) async {
    final r = await http.patch(_u('/api/v1/materials/$id'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(patch));
    if (r.statusCode != 200)
      throw Exception('Fehler: ${r.statusCode} ${utf8.decode(r.bodyBytes)}');
    return jsonDecode(utf8.decode(r.bodyBytes));
  ...
  Future<List<dynamic>> listInvoicesOut(
    ...
    if (q != null && q.isNotEmpty) qp['q'] = q;
    return _getList('/api/v1/ar/invoices', qp.isEmpty ? null : qp);
  }

  Future<Map<String, dynamic>> getInvoiceOut(String id) =>
      _getJson('/api/v1/ar/invoices/$id');
  Future<Map<String, dynamic>> createInvoiceOut(
          Map<String, dynamic> body) async =>
      Map<String, dynamic>.from(await _postJson('/api/v1/ar/invoices/', body));
  Future<Map<String, dynamic>> bookInvoiceOut(String id) async {
    final r = await http.post(_u('/api/v1/ar/invoices/$id/book'));
    if (r.statusCode != 200)
      throw Exception('Fehler: ${r.statusCode} ${utf8.decode(r.bodyBytes)}');
    return jsonDecode(utf8.decode(r.bodyBytes)) as Map<String, dynamic>;
  }
  ...
  Future<List<dynamic>> listInvoicePayments(String id) =>
      _getList('/api/v1/ar/invoices/$id/payments');
  Future<Map<String, dynamic>> addInvoicePayment(
          String id, Map<String, dynamic> body) async =>
      Map<String, dynamic>.from(
          await _postJson('/api/v1/ar/invoices/$id/payments', body));
  Future<List<dynamic>> listBankStatements({int? limit, int? offset}) async {
    final qp = <String, String>{};
    if (limit != null) qp['limit'] = '$limit';
    if (offset != null) qp['offset'] = '$offset';
    return _getList('/api/v1/bank/statements', qp.isEmpty ? null : qp);
  }

  Future<Map<String, dynamic>> matchBankStatement(String stmtId,
          {String? invoiceId}) async =>
      Map<String, dynamic>.from(await _postJson(
          '/api/v1/bank/statements/$stmtId/match',
          invoiceId == null ? {} : {'invoice_id': invoiceId}));

  // -------- HR --------
  Future<List<dynamic>> listEmployees({int? limit, int? offset}) async {
    final qp = <String, String>{};
    if (limit != null) qp['limit'] = '$limit';
    if (offset != null) qp['offset'] = '$offset';
    return _getList('/api/v1/hr/employees', qp.isEmpty ? null : qp);
  }

  Future<Map<String, dynamic>> getEmployee(String id) =>
      _getJson('/api/v1/hr/employees/$id');
  Future<Map<String, dynamic>> createEmployee(
          Map<String, dynamic> body) async =>
      Map<String, dynamic>.from(await _postJson('/api/v1/hr/employees/', body));
  Future<void> updateEmployee(String id, Map<String, dynamic> patch) async {
    final r = await http.patch(_u('/api/v1/hr/employees/$id'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(patch));
    if (r.statusCode != 204) {
      throw Exception('Fehler: ${r.statusCode} ${utf8.decode(r.bodyBytes)}');
    }
  }
  Future<List<dynamic>> listTeams() => _getList('/api/v1/hr/employees/teams');

  Future<List<dynamic>> listLeaveRequests({String? employeeId}) async {
    final qp = <String, String>{};
    if (employeeId != null && employeeId.isNotEmpty)
      qp['employee_id'] = employeeId;
    return _getList('/api/v1/hr/leave-requests', qp.isEmpty ? null : qp);
  }

  Future<Map<String, dynamic>> createLeaveRequest(
          Map<String, dynamic> body) async =>
      Map<String, dynamic>.from(
          await _postJson('/api/v1/hr/leave-requests/', body));
  Future<void> decideLeaveRequest(String id,
      {required String approverId, required bool approve}) async {
    final r = await http.post(_u('/api/v1/hr/leave-requests/$id/decision'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'approver_id': approverId, 'approve': approve}));
    if (r.statusCode != 204) {
      throw Exception('Fehler: ${r.statusCode} ${utf8.decode(r.bodyBytes)}');
    }
  }
  ...
  Future<void> upsertUnit(String code, String name) async {
    final r = await http.post(_u('/api/v1/settings/units/'),
        headers: {'Content-Type': 'application/json'},
  ...
  Future<List<dynamic>> listContacts(
      {String? q, String? rolle, String? typ, int? limit, int? offset}) async {
    final qp = <String, String>{};
    if (q != null && q.isNotEmpty) qp['q'] = q;
  ...
  Future<Map<String, dynamic>> createContact(Map<String, dynamic> body) async {
    final r = await http.post(_u('/api/v1/contacts/'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(body));
    if (r.statusCode != 201) throw Exception(utf8.decode(r.bodyBytes));
    return jsonDecode(utf8.decode(r.bodyBytes));
  }
  ...
  Future<Map<String, dynamic>> updateContact(
      String id, Map<String, dynamic> patch) async {
    final r = await http.patch(_u('/api/v1/contacts/$id'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(patch));
    if (r.statusCode != 200) throw Exception(utf8.decode(r.bodyBytes));
    return jsonDecode(utf8.decode(r.bodyBytes));
  }
  ...
  Future<Map<String, dynamic>> createContactAddress(
    ...
    final r = await http.post(_u('/api/v1/contacts/$contactId/addresses'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(body));
    if (r.statusCode != 201) throw Exception(utf8.decode(r.bodyBytes));
    return jsonDecode(utf8.decode(r.bodyBytes));
  }
  ...
  Future<void> deleteContactAddress(String contactId, String addrId) async {
    final r =
        await http.delete(_u('/api/v1/contacts/$contactId/addresses/$addrId'));
    if (r.statusCode != 204) throw Exception('Fehler: ${r.statusCode}');
  }

  Future<Map<String, dynamic>> createContactPerson(
      String contactId, Map<String, dynamic> body) async {
    final r = await http.post(_u('/api/v1/contacts/$contactId/persons'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(body));
    if (r.statusCode != 201) throw Exception(utf8.decode(r.bodyBytes));
    return jsonDecode(utf8.decode(r.bodyBytes));
  }
  ...
  Future<void> deleteContactPerson(String contactId, String personId) async {
    final r =
        await http.delete(_u('/api/v1/contacts/$contactId/persons/$personId'));
    if (r.statusCode != 204) throw Exception('Fehler: ${r.statusCode}');
  }
  ...
  Future<Map<String, dynamic>> createWarehouse(
      Map<String, dynamic> body) async {
    final r = await http.post(_u('/api/v1/warehouses/'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(body));
    if (r.statusCode != 201) throw Exception(utf8.decode(r.bodyBytes));
    return jsonDecode(utf8.decode(r.bodyBytes));
  }
  ...
  Future<Map<String, dynamic>> createLocation(
    ...
    final r = await http.post(_u('/api/v1/warehouses/$warehouseId/locations'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(body));
    if (r.statusCode != 201) throw Exception(utf8.decode(r.bodyBytes));
    return jsonDecode(utf8.decode(r.bodyBytes));
  }
  ...
  Future<Map<String, dynamic>> createStockMovement(
      Map<String, dynamic> body) async {
    final r = await http.post(_u('/api/v1/stock-movements/'),
        headers: {'Content-Type': 'application/json'}, body: jsonEncode(body));
    if (r.statusCode != 201) throw Exception(utf8.decode(r.bodyBytes));
    return jsonDecode(utf8.decode(r.bodyBytes));
  }
