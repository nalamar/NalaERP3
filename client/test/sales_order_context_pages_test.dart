...
class _FakeApiClient extends ApiClient {
  ...
  @override
  Future<Map<String, dynamic>> getQuote(String id) async => quoteDetail ?? <String, dynamic>{'id': id};
  ...
  Future<List<dynamic>> listInvoicesOut({
    ...
    int? limit,
    int? offset,
  }) async => sourceSalesOrderId != null && sourceSalesOrderId.isNotEmpty ? salesOrderInvoices : invoiceList;

  @override
  Future<Map<String, dynamic>> getInvoiceOut(String id) async => invoiceDetail ?? <String, dynamic>{'id': id};
  ...
  @override
  Future<List<String>> listSalesOrderStatuses() async => const ['open', 'released', 'invoiced'];

  @override
  Future<Map<String, dynamic>> getSalesOrder(String id) async => salesOrderDetail ?? <String, dynamic>{'id': id};
...
void main() {
  testWidgets('QuotesPage shows linked sales order context with formatted value', (tester) async {
  ...
  testWidgets('InvoicesPage shows source sales order context with formatted values', (tester) async {
    final api = _FakeApiClient(
      invoiceList: const [
        {
          'id': 'inv-1',
    ...
    expect(find.text('Auftragsstatus: invoiced  ·  Auftragswert: 1428.00 EUR'), findsOneWidget);
    ...
    expect(find.text('Brutto: 1428.00 EUR  Offen: 1000.00 EUR'), findsOneWidget);
    expect(find.text('Menge 2 x 500.00 EUR  ·  Steuer DE19  ·  Konto 8000'), findsOneWidget);
    expect(find.text('1000.00 EUR'), findsOneWidget);
  });

  testWidgets('SalesOrdersPage shows latest and earlier follow-up invoices separately', (tester) async {
    final api = _FakeApiClient(
      permissions: const {'sales_orders.write', 'invoices_out.write', 'invoices_out.read'},
  ...
  testWidgets('SalesOrdersPage filters list to partial invoices only', (tester) async {
    final api = _FakeApiClient(
      salesOrderList: const [
        {
          'id': 'so-partial',
