...
func (s *Service) Create(ctx context.Context, in ProjectCreate) (*Project, error) {
    ...
    if strings.TrimSpace(in.Status) == "" { in.Status = "neu" }
...
func (s *Service) List(ctx context.Context, f ProjectFilter) ([]Project, error) {
    ...
    if strings.TrimSpace(f.Q) != "" {
        ...
        q := "%" + f.Q + "%"; args = append(args, q, q); idx += 2
    }
    ...
    for rows.Next() {
        var p Project
        if err := rows.Scan(&p.ID, &p.Nummer, &p.Name, &p.KundeID, &p.Status, &p.Angelegt); err != nil { return nil, err }
        out = append(out, p)
    }
    return out, nil
}

func nullIfEmpty(v string) any { if strings.TrimSpace(v) == "" { return nil } ; return v }
...
func (s *Service) ListImports(ctx context.Context, projectID string) ([]ImportRun, error) {
    ...
    for rows.Next() {
        var r ImportRun
        if err := rows.Scan(&r.ID, &r.ProjectID, &r.Source, &r.ImportedAt, &r.CreatedPhases, &r.UpdatedPhases, &r.CreatedElevations, &r.UpdatedElevations, &r.CreatedVariants, &r.UpdatedVariants, &r.DeletedVariants, &r.MaterialsReplaced); err != nil { return nil, err }
        out = append(out, r)
    }
    return out, nil
}
...
func (s *Service) ListImportChanges(ctx context.Context, importID string) ([]ImportChange, error) {
    ...
    for rows.Next() {
        var c ImportChange
        var b, a []byte
        if err := rows.Scan(&c.ID, &c.ImportID, &c.Kind, &c.Action, &c.InternalID, &c.ExternalRef, &c.Message, &b, &a, &c.CreatedAt); err != nil { return nil, err }
...
func (s *Service) ListImportChangesFiltered(ctx context.Context, importID, kind, action string) ([]ImportChange, error) {
    ...
    args := []any{importID}
    if strings.TrimSpace(kind) != "" { q += " AND kind=$2"; args = append(args, kind) }
    if strings.TrimSpace(action) != "" { if len(args) == 1 { q += " AND action=$2"; args = append(args, action) } else { q += " AND action=$3"; args = append(args, action) } }
...
func (s *Service) CreatePhase(ctx context.Context, projectID string, in PhaseCreate) (*Phase, error) {
    if strings.TrimSpace(in.Nummer) == "" { in.Nummer = "1" }
    if strings.TrimSpace(in.Name) == "" { return nil, errors.New("Name erforderlich") }
    id := uuid.NewString()
    var p Phase
    err := s.pg.QueryRow(ctx, `
...
func (s *Service) ListPhases(ctx context.Context, projectID string) ([]Phase, error) {
    ...
    for rows.Next() {
        var p Phase
        if err := rows.Scan(&p.ID, &p.ProjectID, &p.Nummer, &p.Name, &p.Beschreibung, &p.SortOrder, &p.Angelegt); err != nil { return nil, err }
        out = append(out, p)
    }
    return out, nil
}
...
func (s *Service) UpdatePhase(ctx context.Context, id string, u PhaseUpdate) (*Phase, error) {
    ...
    if u.Nummer != nil { if strings.TrimSpace(*u.Nummer) == "" { return nil, errors.New("Nummer erforderlich") }; add("nummer", *u.Nummer) }
    if u.Name != nil { if strings.TrimSpace(*u.Name) == "" { return nil, errors.New("Name erforderlich") }; add("name", *u.Name) }
    if u.Beschreibung != nil { add("beschreibung", nullIfEmpty(*u.Beschreibung)) }
    if u.SortOrder != nil { add("sort_order", *u.SortOrder) }
    if len(sets) == 0 { return s.GetPhase(ctx, id) }
    args = append(args, id)
...
func (s *Service) DeletePhase(ctx context.Context, id string) error {
    if _, err := s.pg.Exec(ctx, `DELETE FROM project_phases WHERE id=$1`, id); err != nil { return err }
    return nil
}
...
func (s *Service) ListElevations(ctx context.Context, phaseID string) ([]Elevation, error) {
    ...
    for rows.Next() {
        var e Elevation
        if err := rows.Scan(&e.ID, &e.PhaseID, &e.Nummer, &e.Name, &e.Beschreibung, &e.Menge, &e.WidthMM, &e.HeightMM, &e.ExternalGUID, &e.Serie, &e.Oberflaeche, &e.Picture1Rel, &e.Angelegt); err != nil { return nil, err }
        out = append(out, e)
    }
    return out, nil
}
...
func (s *Service) UpdateElevation(ctx context.Context, id string, u ElevationUpdate) (*Elevation, error) {
    ...
    if u.Name != nil { if strings.TrimSpace(*u.Name) == "" { return nil, errors.New("Name erforderlich") }; add("name", *u.Name) }
    if u.Beschreibung != nil { add("beschreibung", nullIfEmpty(*u.Beschreibung)) }
    if u.Menge != nil { add("menge", *u.Menge) }
    if u.WidthMM != nil { add("width_mm", *u.WidthMM) }
    if u.HeightMM != nil { add("height_mm", *u.HeightMM) }
    if u.ExternalGUID != nil { add("external_guid", nullIfEmpty(*u.ExternalGUID)) }
    if u.Serie != nil { add("serie", nullIfEmpty(*u.Serie)) }
    if u.Oberflaeche != nil { add("oberflaeche", nullIfEmpty(*u.Oberflaeche)) }
    if len(sets) == 0 { return s.GetElevation(ctx, id) }
    args = append(args, id)
...
func (s *Service) DeleteElevation(ctx context.Context, id string) error {
    if _, err := s.pg.Exec(ctx, `DELETE FROM project_elevations WHERE id=$1`, id); err != nil { return err }
    return nil
}
...
func (s *Service) ListSingleElevations(ctx context.Context, elevationID string) ([]SingleElevation, error) {
    ...
    for rows.Next() {
        var se SingleElevation
        if err := rows.Scan(&se.ID, &se.ElevationID, &se.Name, &se.Beschreibung, &se.Menge, &se.Selected, &se.ExternalGUID, &se.Angelegt); err != nil { return nil, err }
        out = append(out, se)
    }
    return out, nil
}
...
func (s *Service) UpdateSingleElevation(ctx context.Context, id string, u SingleElevationUpdate) (*SingleElevation, error) {
    ...
    if u.Beschreibung != nil { add("beschreibung", nullIfEmpty(*u.Beschreibung)) }
    if u.Menge != nil { add("menge", *u.Menge) }
    if u.Selected != nil { add("selected", *u.Selected) }
    if u.ExternalGUID != nil { add("external_guid", nullIfEmpty(*u.ExternalGUID)) }
    if len(sets) == 0 { return s.GetSingleElevation(ctx, id) }
    args = append(args, id)
...
func (s *Service) DeleteSingleElevation(ctx context.Context, id string) error {
    if _, err := s.pg.Exec(ctx, `DELETE FROM project_single_elevations WHERE id=$1`, id); err != nil { return err }
    return nil
}
...
func (s *Service) ListProfilesBySingle(ctx context.Context, singleID string) ([]SingleProfile, error) {
    ...
    for rows.Next() {
        ...
        if err := rows.Scan(&it.ID, &it.SingleElevation, &it.SupplierCode, &it.ArticleCode, &it.Description, &it.LengthMM, &it.Qty, &it.Unit, &it.MaterialID, &it.MaterialNummer, &it.MaterialBez); err != nil { return nil, err }
        out = append(out, it)
    }
    return out, nil
}
...
func (s *Service) ListArticlesBySingle(ctx context.Context, singleID string) ([]SingleArticle, error) {
    ...
    for rows.Next() {
        ...
        if err := rows.Scan(&it.ID, &it.SingleElevation, &it.SupplierCode, &it.ArticleCode, &it.Description, &it.Qty, &it.Unit, &it.MaterialID, &it.MaterialNummer, &it.MaterialBez); err != nil { return nil, err }
        out = append(out, it)
    }
    return out, nil
}
...
func (s *Service) ListGlassBySingle(ctx context.Context, singleID string) ([]SingleGlass, error) {
    ...
    for rows.Next() {
        var it SingleGlass
        if err := rows.Scan(&it.ID, &it.SingleElevation, &it.Configuration, &it.Description, &it.WidthMM, &it.HeightMM, &it.AreaM2, &it.Qty, &it.Unit, &it.MaterialID, &it.MaterialNummer, &it.MaterialBez); err != nil { return nil, err }
        out = append(out, it)
    }
    return out, nil
}
...
func (s *Service) LinkVariantMaterial(ctx context.Context, kind, itemID, materialID string) error {
    if strings.TrimSpace(itemID) == "" || strings.TrimSpace(kind) == "" { return errors.New("Ungültige Parameter") }
    isUnlink := strings.TrimSpace(materialID) == ""
    if !isUnlink {
        var dummy string
        if err := s.pg.QueryRow(ctx, `SELECT id FROM materials WHERE id=$1`, materialID).Scan(&dummy); err != nil { return errors.New("Material nicht gefunden") }
    }
    var q string
    switch strings.ToLower(kind) {
    case "profiles": q = `UPDATE single_elevation_profiles SET material_id=$1 WHERE id=$2`
    case "articles": q = `UPDATE single_elevation_articles SET material_id=$1 WHERE id=$2`
    case "glass":    q = `UPDATE single_elevation_glass SET material_id=$1 WHERE id=$2`
    default: return errors.New("Ungültiger Typ")
    }
    var arg any
    if isUnlink { arg = nil } else { arg = materialID }
    if _, err := s.pg.Exec(ctx, q, arg, itemID); err != nil { return err }
    return nil
}
...
func (s *Service) ListProjectAssets(ctx context.Context, projectID string) ([]ProjectAsset, error) {
    ...
    for rows.Next() {
        var a ProjectAsset
        if err := rows.Scan(&a.RelPath, &a.ContentType, &a.Length, &a.UploadedAt); err != nil { return nil, err }
        out = append(out, a)
    }
    return out, nil
}
