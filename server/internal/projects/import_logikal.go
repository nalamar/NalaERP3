...
func (s *Service) ImportLogikal(ctx context.Context, sqlitePath string, sourceName string, companyID string) (*Project, string, error) {
	...
	if nummer == "" {
		nummer = strings.TrimSpace(projOrderNo)
	}
	if nummer == "" {
		nummer = projGUID
	}
	if nummer == "" {
		...
		if n, e := settings.NewNumberingService(s.pg).Next(ctx, "project", companyID); e == nil {
			nummer = n
		} else {
			nummer = "PRJ-"
		}
	...
	if err := s.pg.QueryRow(ctx, `SELECT id FROM projects WHERE nummer=$1 AND company_id=$2`, nummer, companyID).Scan(&existingID); err == nil && existingID != "" {
		// update name/status
		if _, err := s.pg.Exec(ctx, `UPDATE projects SET name=$1, status=$2 WHERE id=$3 AND company_id=$4`, defaultString(projName, "Logikal-Projekt"), "importiert", existingID, companyID); err != nil {
			return nil, "", fmt.Errorf("projekt update: %w", err)
		}
		p, err = s.Get(ctx, existingID, companyID)
		if err != nil {
			return nil, "", fmt.Errorf("projekt laden: %w", err)
		}
	} else {
		p, err = s.Create(ctx, ProjectCreate{Nummer: nummer, Name: defaultString(projName, "Logikal-Projekt"), Status: "importiert"}, companyID)
		if err != nil {
			return nil, "", fmt.Errorf("projekt anlegen: %w", err)
		}
	}
	...
	snapElevation := func(id string) map[string]any {
		...
		if err := s.pg.QueryRow(ctx, `SELECT id, phase_id, nummer, name, COALESCE(beschreibung,''), menge, width_mm, height_mm, COALESCE(external_guid,'') FROM project_elevations WHERE id=$1`, id).Scan(&e.ID, &e.PhaseID, &e.Nummer, &e.Name, &e.Beschreibung, &e.Menge, &e.WidthMM, &e.HeightMM, &e.ExternalGUID); err == nil {
			...
			if e.WidthMM != nil {
				m["width_mm"] = *e.WidthMM
			}
			if e.HeightMM != nil {
				m["height_mm"] = *e.HeightMM
			}
			return m
		}
		return nil
	...
	if rows, err := db.QueryContext(ctx, `SELECT ElevationGroupID, PhaseId FROM ElevationGroups`); err == nil {
		for rows.Next() {
			...
			if e := rows.Scan(&gid, &pid); e == nil {
				if gid.Valid && pid.Valid {
					groupToPhase[gid.Int64] = pid.Int64
					phaseIDs[pid.Int64] = struct{}{}
				}
			}
		}
		_ = rows.Close()
	...
	if len(phaseIDs) == 0 {
		ph, err := s.CreatePhase(ctx, p.ID, PhaseCreate{Nummer: "1", Name: "Los 1", SortOrder: 0}, companyID)
		if err != nil {
			return nil, "", fmt.Errorf("standard-phase: %w", err)
		}
		...
		for pid := range phaseIDs {
			...
			if n, ok := namesByPhaseID[pid]; ok && strings.TrimSpace(n) != "" {
				desiredName = n
			}
			var ph Phase
			if err := s.pg.QueryRow(ctx, `SELECT id, project_id, nummer, name, COALESCE(beschreibung,''), sort_order, angelegt_am FROM project_phases WHERE project_id=$1 AND nummer=$2`, p.ID, num).Scan(
				...
				ph2, err := s.CreatePhase(ctx, p.ID, PhaseCreate{Nummer: num, Name: desiredName, SortOrder: i}, companyID)
				if err != nil {
					return nil, "", fmt.Errorf("phase %d: %w", pid, err)
				}
	...
	if err != nil {
		return nil, "", fmt.Errorf("elevations lesen: %w", err)
	}
	elevs := make([]elevRow, 0)
	for rows.Next() {
		var r elevRow
		if err := rows.Scan(&r.id, &r.groupID, &r.name, &r.guid, &r.amount, &r.width, &r.height, &r.alt); err != nil {
			return nil, "", err
		}
	...
	for _, r := range elevs {
		...
		if elevID == "" {
			...
			e, err := s.CreateElevation(ctx, p.ID, ph.ID, ElevationCreate{Nummer: n, Name: defaultString(r.name, fmt.Sprintf("Pos %s", n)), Beschreibung: "", Menge: r.amount, WidthMM: w, HeightMM: h, ExternalGUID: r.guid}, companyID)
			if err != nil {
				return nil, "", fmt.Errorf("elevation anlegen: %w", err)
			}
			elevID = e.ID
			cnt.createdElevs++
	...
	if err != nil {
		return nil, "", fmt.Errorf("single elevations lesen: %w", err)
	}
	...
	for srows.Next() {
		...
		var amt float64
		var sysLong, sysName, sysCode, pic1 string
		if err := srows.Scan(&sid, &gid, &name, &guid, &amt, &sysLong, &sysName, &sysCode, &pic1); err != nil {
			return nil, "", err
		}
		...
		if strings.TrimSpace(guid) != "" {
			_ = s.pg.QueryRow(ctx, `SELECT id FROM project_single_elevations WHERE elevation_id=$1 AND external_guid=$2`, elevID, guid).Scan(&seID)
		}
		if seID == "" && strings.TrimSpace(name) != "" {
			_ = s.pg.QueryRow(ctx, `SELECT id FROM project_single_elevations WHERE elevation_id=$1 AND name=$2 LIMIT 1`, elevID, name).Scan(&seID)
		}
		if seID == "" {
			se, err := s.CreateSingleElevation(ctx, p.ID, elevID, SingleElevationCreate{Name: defaultString(name, "Variante"), Menge: amt, Selected: false, ExternalGUID: guid}, companyID)
			if err != nil {
				return nil, "", fmt.Errorf("variante anlegen: %w", err)
			}
			seID = se.ID
			cnt.createdVars++
		...
		if _, ok := seriesByGroup[gid]; !ok {
			...
			if s == "" {
				s = strings.TrimSpace(sysName)
			}
			if s == "" {
				s = strings.TrimSpace(sysCode)
			}
			if s != "" {
				seriesByGroup[gid] = s
			}
	...
	if rows, err := db.QueryContext(ctx, `SELECT ElevationId, COALESCE(SurfaceColor,'') FROM ElevationSurface`); err == nil {
		...
		for rows.Next() {
			var extEID int64
			var color string
			if err := rows.Scan(&extEID, &color); err != nil {
				return nil, "", err
			}
			...
			if internalID, ok := elevIDMap[extEID]; ok && internalID != "" {
				if _, done := written[internalID]; done {
					continue
				}
	...
	if err == nil {
		...
		for insRows.Next() {
			var iid int64
			var guid string
			if err := insRows.Scan(&iid, &guid); err != nil {
				return nil, "", err
			}
			if guid != "" {
				insByGUID[guid] = iid
		...
		if insRows2, e2 := db.QueryContext(ctx, `SELECT InsertionID, COALESCE(xGUID,''), COALESCE(GroupID,0) FROM Insertions`); e2 == nil {
			for insRows2.Next() {
				...
				var guid string
				var gid int64
				if err := insRows2.Scan(&iid, &guid, &gid); err == nil {
					insGroup[iid] = gid
					if guid != "" {
						if seID, ok := singleByGUID[guid]; ok {
							seByInsertion[iid] = seID
						}
					}
				}
			}
		...
		getSeForInsertion := func(insID int64) string {
			if seID, ok := seByInsertion[insID]; ok && seID != "" {
				return seID
			}
			if gid, ok := insGroup[insID]; ok {
				if elevID := mainElevationByGroup[gid]; elevID != "" {
					...
					if se, err := s.CreateSingleElevation(ctx, p.ID, elevID, SingleElevationCreate{Name: "Variante", Menge: 1, Selected: false}, companyID); err == nil {
						return se.ID
					}
				}
			}
		...
		autoLink := func(kind, itemID, supplier, code string) {
			...
			case "profiles":
				_, _ = s.pg.Exec(ctx, `UPDATE single_elevation_profiles SET material_id=$1 WHERE id=$2`, mid, itemID)
			case "articles":
				_, _ = s.pg.Exec(ctx, `UPDATE single_elevation_articles SET material_id=$1 WHERE id=$2`, mid, itemID)
			case "glass":
				_, _ = s.pg.Exec(ctx, `UPDATE single_elevation_glass SET material_id=$1 WHERE id=$2`, mid, itemID)
			}
		}
		...
		if rows, err := db.QueryContext(ctx, `SELECT InsertionId, COALESCE(ArticleCode,''), COALESCE(Description,''), COALESCE(CAST(LK_SupplierID AS TEXT),''), COALESCE(Length,0), COALESCE(Amount,0), COALESCE(Length_Unit,'') FROM Profiles`); err == nil {
			for rows.Next() {
				var insID int64
				var code, desc, supplier string
				var length float64
				var amt int64
				var unit string
				if err := rows.Scan(&insID, &code, &desc, &supplier, &length, &amt, &unit); err != nil {
					return nil, "", err
				}
				seID := getSeForInsertion(insID)
				if seID == "" {
					continue
				}
				...
				if _, err := s.pg.Exec(ctx, `INSERT INTO single_elevation_profiles (id, single_elevation_id, supplier_code, article_code, description, length_mm, qty, unit) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
					iid, seID, nullIfEmpty(supplier), nullIfEmpty(code), nullIfEmpty(desc), nullIfZeroFloat(length), nullIfZeroInt(amt), nullIfEmpty(unit)); err != nil {
					return nil, "", err
				}
		...
		if rows, err := db.QueryContext(ctx, `SELECT InsertionId, COALESCE(ArticleCode,''), COALESCE(Description,''), COALESCE(CAST(LK_SupplierId AS TEXT),''), COALESCE(Amount,0), COALESCE(Units_Unit,'') FROM Articles`); err == nil {
			for rows.Next() {
				...
				var amt int64
				var unit string
				if err := rows.Scan(&insID, &code, &desc, &supplier, &amt, &unit); err != nil {
					return nil, "", err
				}
				seID := getSeForInsertion(insID)
				if seID == "" {
				...
				if _, err := s.pg.Exec(ctx, `INSERT INTO single_elevation_articles (id, single_elevation_id, supplier_code, article_code, description, qty, unit) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
					iid, seID, nullIfEmpty(supplier), nullIfEmpty(code), nullIfEmpty(desc), nullIfZeroInt(amt), nullIfEmpty(unit)); err != nil {
					return nil, "", err
				}
		...
		if rows, err := db.QueryContext(ctx, `SELECT InsertionId, COALESCE(Configuration,''), COALESCE(Description,''), 1 FROM Glass`); err == nil {
			for rows.Next() {
				var insID int64
				var conf, desc string
				var qty int
				if err := rows.Scan(&insID, &conf, &desc, &qty); err != nil {
					return nil, "", err
				}
				seID := getSeForInsertion(insID)
				if seID == "" {
				...
				if _, err := s.pg.Exec(ctx, `INSERT INTO single_elevation_glass (id, single_elevation_id, configuration, description, qty) VALUES ($1,$2,$3,$4,$5)`,
					iid, seID, nullIfEmpty(conf), nullIfEmpty(desc), qty); err != nil {
					return nil, "", err
				}
	...
	for pid, ph := range phaseByPhaseID {
		if ph != nil {
			currentPhaseIDs = append(currentPhaseIDs, ph.ID)
		}
...
func toRelativeAssetPath(p string) string {
	...
	if strings.HasPrefix(strings.ToLower(s), "emfs/") || strings.HasPrefix(strings.ToLower(s), "rtfs/") {
		return s
	}
	return ""
}
