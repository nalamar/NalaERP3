...
func (s *Service) AnalyzeLogikal(ctx context.Context, sqlitePath string) (map[string]any, error) {
		...
		for rows.Next() {
			...
			if e := rows.Scan(&gid, &pid); e == nil {
				if gid.Valid && pid.Valid {
					grpToPhase[gid.Int64] = pid.Int64
					phaseSet[pid.Int64] = struct{}{}
				}
			}
		}
		_ = rows.Close()
	...
	if rows, err := db.QueryContext(ctx, `SELECT ElevationID, ElevationGroupId, COALESCE(Name,''), COALESCE(xGUID,''), COALESCE(Amount,0) FROM Elevations`); err == nil {
		for rows.Next() {
			...
			var pid int64 = -1
			if gid.Valid {
				if p, ok := grpToPhase[gid.Int64]; ok {
					pid = p
				}
			}
			ph := phases[pid]
			if ph == nil {
				ph = map[string]any{"phase_id": pid, "elevation_groups": map[int64]int{}, "elevations": 0}
				phases[pid] = ph
			}
			// count elevations
    ...
    `); err == nil {
		...
		for rows.Next() {
			...
			b := best[pid.Int64]
			if c > b.cnt {
				best[pid.Int64] = agg{name: t, cnt: c}
			}
		}
		_ = rows.Close()
		for pid, b := range best {
			if strings.TrimSpace(b.name) != "" {
				phaseName[pid] = b.name
			}
		}
	}
