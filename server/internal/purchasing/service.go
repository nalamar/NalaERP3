...
// WithAudit aktiviert das generische Aenderungsprotokoll (Subtask 0.3.3.4)
// fuer Statuswechsel. Ohne Aufruf bleibt s.audit nil und die
// Protokollaufrufe werden uebersprungen (wichtig fuer bestehende Tests,
...
// isEditableStatus: nur draft-Bestellungen duerfen inhaltlich veraendert
// werden (Festschreibung fuer versendete/abgeschlossene Belege, Task 0.3.2).
// Der Statuswechsel selbst (z.B. draft->ordered) bleibt unabhaengig davon
// ueber Update() moeglich - nur INHALTSfelder (Nummer, Datum, Waehrung,
// Notiz, Positionen) werden ab ordered/received/canceled schreibgeschuetzt.
...
func (s *Service) Create(ctx context.Context, in PurchaseOrderCreate, companyID string) (*PurchaseOrder, []PurchaseOrderItem, error) {
	...
	if !isIn(in.Status, Statuses()) {
		return nil, nil, errors.New("Ungültiger Status")
	}
	...
	if strings.TrimSpace(in.Number) == "" {
		...
		if n, err := numSvc.Next(ctx, "purchase_order", companyID); err == nil {
			in.Number = n
		}
	}
    ...
    `, id, in.SupplierID, in.Number, date, in.Currency, in.Status, in.Note, companyID).Scan(
...
func (s *Service) Get(ctx context.Context, id string, companyID string) (*PurchaseOrder, []PurchaseOrderItem, error) {
	var po PurchaseOrder
	if err := s.pg.QueryRow(ctx, `SELECT id, supplier_id, number, order_date, currency, status, COALESCE(note,''), angelegt_am FROM purchase_orders WHERE id=$1 AND company_id=$2`, id, companyID).Scan(
		&po.ID, &po.SupplierID, &po.Number, &po.OrderDate, &po.Currency, &po.Status, &po.Note, &po.Angelegt,
	); err != nil {
		return nil, nil, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&it.ID, &it.OrderID, &it.Position, &it.MaterialID, &it.Description, &it.Qty, &it.UOM, &it.UnitPrice, &it.Currency, &it.DeliveryDate); err != nil {
			return &po, nil, err
		}
...
func (s *Service) List(ctx context.Context, f PurchaseOrderFilter, companyID string) ([]PurchaseOrder, error) {
	lim := f.Limit
	if lim <= 0 || lim > 200 {
		lim = 50
	}
	...
	if strings.TrimSpace(f.Q) != "" {
		conds = append(conds, fmt.Sprintf("(number ILIKE $%d)", idx))
		args = append(args, "%"+f.Q+"%")
		idx++
	}
	if strings.TrimSpace(f.SupplierID) != "" {
		conds = append(conds, fmt.Sprintf("supplier_id=$%d", idx))
	...
	for rows.Next() {
		var po PurchaseOrder
		if err := rows.Scan(&po.ID, &po.SupplierID, &po.Number, &po.OrderDate, &po.Currency, &po.Status, &po.Note, &po.Angelegt); err != nil {
			return nil, err
		}
		out = append(out, po)
	}
...
func (s *Service) Update(ctx context.Context, id string, u PurchaseOrderUpdate, companyID string, actorUserID string) (*PurchaseOrder, []PurchaseOrderItem, error) {
	contentChange := u.Number != nil || u.OrderDate != nil || u.Currency != nil || u.Note != nil
	...
	if u.OrderDate != nil {
		add("order_date", *u.OrderDate)
	}
	if u.Currency != nil {
		add("currency", strings.ToUpper(strings.TrimSpace(*u.Currency)))
	}
	if u.Status != nil {
		if !isIn(*u.Status, Statuses()) {
			return nil, nil, errors.New("Ungültiger Status")
		}
		add("status", *u.Status)
	}
	...
	if err := tx.QueryRow(ctx, `SELECT status FROM purchase_orders WHERE id=$1 AND company_id=$2 FOR UPDATE`, id, companyID).Scan(&currentStatus); err != nil {
		return nil, nil, err
	}
	if contentChange && !isEditableStatus(currentStatus) {
		return nil, nil, fmt.Errorf("Bestellung ist nicht im Status draft und kann nicht mehr inhaltlich geändert werden (aktuell: %s)", currentStatus)
	}

	args = append(args, id, companyID)
	q := fmt.Sprintf("UPDATE purchase_orders SET %s WHERE id=$%d AND company_id=$%d", strings.Join(sets, ", "), idx, idx+1)
	if _, err := tx.Exec(ctx, q, args...); err != nil {
		return nil, nil, err
	}
	if s.audit != nil && u.Status != nil && currentStatus != *u.Status {
		if err := s.audit.Record(ctx, tx, companyID, auditlog.RecordInput{
			...
			After:       map[string]any{"status": *u.Status},
		}); err != nil {
			return nil, nil, err
		}
	}
...
func (s *Service) CreateItem(ctx context.Context, orderID string, in PurchaseOrderItemInput, companyID string) (*PurchaseOrderItem, error) {
	if strings.TrimSpace(in.MaterialID) == "" || in.Qty == 0 || strings.TrimSpace(in.UOM) == "" {
		return nil, errors.New("Ungültige Position")
	}
	if in.Currency == "" {
		in.Currency = "EUR"
	}
	order, _, err := s.Get(ctx, orderID, companyID)
	if err != nil {
		return nil, err
	}
	if !isEditableStatus(order.Status) {
		return nil, fmt.Errorf("Bestellung ist nicht im Status draft und kann nicht mehr inhaltlich geändert werden (aktuell: %s)", order.Status)
	}
	id := uuid.NewString()
	var it PurchaseOrderItem
...
func (s *Service) UpdateItem(ctx context.Context, orderID, itemID string, u PurchaseOrderItemUpdate, companyID string) (*PurchaseOrderItem, error) {
	sets := make([]string, 0)
	args := make([]any, 0)
	idx := 1
	add := func(field string, v any) {
	...
	if u.UnitPrice != nil {
		add("unit_price", *u.UnitPrice)
	}
	if u.Currency != nil {
		add("currency", strings.ToUpper(strings.TrimSpace(*u.Currency)))
	}
	if u.DeliveryDate != nil {
		add("delivery_date", *u.DeliveryDate)
	}
	order, _, err := s.Get(ctx, orderID, companyID)
	if err != nil {
		return nil, err
...
func (s *Service) DeleteItem(ctx context.Context, orderID, itemID string, companyID string) error {
	order, _, err := s.Get(ctx, orderID, companyID)
	if err != nil {
		return err
	}
	if !isEditableStatus(order.Status) {
		return fmt.Errorf("Bestellung ist nicht im Status draft und kann nicht mehr inhaltlich geändert werden (aktuell: %s)", order.Status)
	}
