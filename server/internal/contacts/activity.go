...
func (s *Service) ListActivity(ctx context.Context, contactID string, companyID string) ([]ActivityItem, error) {
	c, err := s.Get(ctx, contactID, companyID)
	if err != nil {
		return nil, err
	}

	notes, err := s.ListNotes(ctx, contactID, companyID)
	if err != nil {
		return nil, err
	}
	tasks, err := s.ListTasks(ctx, contactID, companyID)
	if err != nil {
		return nil, err
	}
	docs, err := s.ListContactDocuments(ctx, contactID, companyID)
	if err != nil {
		return nil, err
	}
