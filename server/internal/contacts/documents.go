...
func (s *Service) UploadContactDocument(ctx context.Context, contactID string, r io.Reader, filename, contentType string, companyID string) (*Document, error) {
...
func (s *Service) ListContactDocuments(ctx context.Context, contactID string, companyID string) ([]Document, error) {
	if _, err := s.Get(ctx, contactID, companyID); err != nil {
		return nil, err
	}
...
// contact_documents-Eintrag verknüpft ist, dessen Kontakt zum Mandanten
...
// Mandanten und sogar ohne jede Verknüpfung zu einem contact_documents-
// Eintrag.
func (s *Service) OpenDocumentStream(ctx context.Context, documentID string, companyID string) (io.ReadCloser, string, string, int64, error) {
	...
	err = s.pg.QueryRow(ctx, `
        SELECT cd.filename, cd.content_type, cd.length
          FROM contact_documents cd
          JOIN contacts c ON c.id = cd.contact_id
         WHERE cd.document_id = $1 AND c.company_id = $2
         LIMIT 1
