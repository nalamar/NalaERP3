...
func PersonRoles() []string { return []string{"management","purchasing","sales","accounting","project","technical","other"} }
func CommunicationChannels() []string { return []string{"email","phone","mobile","whatsapp","teams","other"} }
...
func (s *Service) ensureNoDuplicate(ctx context.Context, excludeID, name, email, vatID, debtorNo, creditorNo string) error {
    if s.pg == nil {
        return nil
    }
...
func (s *Service) Create(ctx context.Context, in ContactCreate) (*Contact, error) {
    ...
    if !isIn(status, Statuses()) { return nil, errors.New("Ungültiger Status") }
    ...
    if strings.TrimSpace(in.SteuerLand) == "" { in.SteuerLand = "DE" }
    in.SteuerLand = strings.ToUpper(strings.TrimSpace(in.SteuerLand))
    if err := s.ensureNoDuplicate(ctx, "", in.Name, in.Email, in.UStID, in.DebitorNr, in.KreditorNr); err != nil { return nil, err }
    id := uuid.NewString()
    var c Contact
    err := s.pg.QueryRow(ctx, `
...
func (s *Service) Update(ctx context.Context, id string, u ContactUpdate) (*Contact, error) {
    ...
    if statusValue != nil {
        ...
        if !isIn(normalized, Statuses()) { return nil, errors.New("Ungültiger Status") }
    ...
    if u.Zahlungsbedingungen != nil { add("payment_terms", *u.Zahlungsbedingungen) }
    if u.DebitorNr != nil {
    ...
    if u.SteuerLand != nil { add("tax_country", strings.ToUpper(strings.TrimSpace(*u.SteuerLand))) }
    if u.Steuerbefreit != nil { add("tax_exempt", *u.Steuerbefreit) }
    if aktivValue != nil {
        normalizedStatus, normalizedAktiv := normalizeStatusAndActive(nil, aktivValue)
        add("aktiv", normalizedAktiv)
        add("status", normalizedStatus)
    }
    if err := s.ensureNoDuplicate(ctx, id, effectiveName, effectiveEmail, effectiveVAT, effectiveDebtorNo, effectiveCreditorNo); err != nil { return nil, err }
...
func (s *Service) List(ctx context.Context, f ContactFilter) ([]Contact, error) {
    ...
    if strings.TrimSpace(f.Q) != "" {
        conds = append(conds, fmt.Sprintf("(name ILIKE $%d OR email ILIKE $%d OR phone ILIKE $%d OR vat_id ILIKE $%d OR debtor_no ILIKE $%d OR creditor_no ILIKE $%d)", idx, idx+1, idx+2, idx+3, idx+4, idx+5))
        q := "%" + f.Q + "%"; args = append(args, q, q, q, q, q, q); idx += 6
    }
    ...
    for rows.Next() {
        var c Contact
        if err := rows.Scan(&c.ID, &c.Typ, &c.Rolle, &c.Status, &c.Name, &c.Email, &c.Telefon, &c.UStID, &c.SteuerNr, &c.Waehrung, &c.Zahlungsbedingungen, &c.DebitorNr, &c.KreditorNr, &c.SteuerLand, &c.Steuerbefreit, &c.Aktiv, &c.Angelegt); err != nil { return nil, err }
        out = append(out, c)
    }
    return out, nil
}
...
func (s *Service) CreatePerson(ctx context.Context, contactID string, in PersonCreate) (*Person, error) {
    ...
    if in.Rolle == "" { in.Rolle = "other" }
    if !isIn(in.Rolle, PersonRoles()) { return nil, errors.New("Ungültige Ansprechpartnerrolle") }
    in.BevorzugterKanal = strings.TrimSpace(strings.ToLower(in.BevorzugterKanal))
    if in.BevorzugterKanal != "" && !isIn(in.BevorzugterKanal, CommunicationChannels()) { return nil, errors.New("Ungültiger Kommunikationskanal") }
    id := uuid.NewString()
    var p Person
    err := s.pg.QueryRow(ctx, `
...
func (s *Service) ListPersons(ctx context.Context, contactID string) ([]Person, error) {
    ...
    for rows.Next() {
        var p Person
        if err := rows.Scan(&p.ID, &p.ContactID, &p.Anrede, &p.Vorname, &p.Nachname, &p.Position, &p.Rolle, &p.BevorzugterKanal, &p.Email, &p.Telefon, &p.Mobil, &p.Primary); err != nil { return nil, err }
        out = append(out, p)
    }
    return out, nil
}
...
func (s *Service) UpdatePerson(ctx context.Context, contactID, personID string, u PersonUpdate) (*Person, error) {
    ...
    if u.Rolle != nil {
        ...
        if role == "" { role = "other" }
        if !isIn(role, PersonRoles()) { return nil, errors.New("Ungültige Ansprechpartnerrolle") }
        add("rolle", role)
    }
    if u.BevorzugterKanal != nil {
        channel := strings.TrimSpace(strings.ToLower(*u.BevorzugterKanal))
        if channel != "" && !isIn(channel, CommunicationChannels()) { return nil, errors.New("Ungültiger Kommunikationskanal") }
...
func (s *Service) ListNotes(ctx context.Context, contactID string) ([]Note, error) {
    ...
    for rows.Next() {
        var n Note
        if err := rows.Scan(&n.ID, &n.ContactID, &n.Titel, &n.Inhalt, &n.ErstelltAm, &n.AktualisiertAm); err != nil { return nil, err }
        out = append(out, n)
    }
    return out, nil
}
...
func (s *Service) UpdateNote(ctx context.Context, contactID, noteID string, u NoteUpdate) (*Note, error) {
    ...
    idx := 1
    add := func(field string, v any) { sets = append(sets, fmt.Sprintf("%s=$%d", field, idx)); args = append(args, v); idx++ }
    if u.Titel != nil { add("titel", strings.TrimSpace(*u.Titel)) }
    if u.Inhalt != nil { add("inhalt", strings.TrimSpace(*u.Inhalt)) }
    if len(sets) == 0 { return s.getNote(ctx, noteID) }
