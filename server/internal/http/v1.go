...
func NewV1Router(pg *pgxpool.Pool, mg *mongo.Client, rd *redis.Client, cfg *config.Config) http.Handler {
	...
	accSvc := settings.NewAccountingService(pg)
	...
	empSvc := hr.NewEmployeeService(pg)
	leaveSvc := hr.NewLeaveService(pg)
	...
	r.Route("/projects", func(r chi.Router) {
		...
		r.Post("/import/logikal", func(w http.ResponseWriter, req *http.Request) {
			...
			if err != nil {
				http.Error(w, "Import fehlgeschlagen: "+err.Error(), http.StatusBadRequest)
				return
			}
		...
		r.Post("/{id}/assets", func(w http.ResponseWriter, req *http.Request) {
			...
			if err != nil {
				http.Error(w, "Upload lesen fehlgeschlagen", http.StatusBadRequest)
				return
			}
			zr, err := zip.NewReader(bytes.NewReader(buf), int64(len(buf)))
			if err != nil {
				http.Error(w, "Ungültiges ZIP", http.StatusBadRequest)
				return
			}
			...
			if err != nil {
				http.Error(w, "GridFS nicht verfügbar", http.StatusInternalServerError)
				return
			}
			type item struct {
				Rel string `json:"rel"`
		...
		r.Get("/{id}/assets", func(w http.ResponseWriter, req *http.Request) {
			...
			if err != nil {
				http.Error(w, "Asset nicht gefunden", http.StatusNotFound)
				return
			}
			...
			if err != nil {
				http.Error(w, "Stream fehlgeschlagen", http.StatusInternalServerError)
				return
			}
			defer rc.Close()
			if contentType == "" {
	...
	r.Route("/settings/numbering", func(r chi.Router) {
		...
		r.Put("/{entity}", func(w http.ResponseWriter, req *http.Request) {
			...
			if strings.TrimSpace(in.Pattern) == "" {
				http.Error(w, "Pattern erforderlich", http.StatusBadRequest)
				return
			}
	...
	r.Route("/settings/pdf", func(r chi.Router) {
		...
		r.Post("/{entity}/upload/{kind}", func(w http.ResponseWriter, req *http.Request) {
			...
			file, header, err := req.FormFile("file")
			if err != nil {
				http.Error(w, "Datei fehlt (file)", http.StatusBadRequest)
				return
			}
			defer file.Close()
			// Upload to GridFS
	...
	// Einstellungen – Konten und Steuerkennzeichen (nur Lesen)
	r.Get("/settings/accounts", func(w http.ResponseWriter, req *http.Request) {
		list, err := accSvc.ListAccounts(req.Context())
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	r.Get("/settings/tax-codes", func(w http.ResponseWriter, req *http.Request) {
		list, err := accSvc.ListTaxCodes(req.Context())
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	r.Route("/ar/invoices", func(r chi.Router) {
		r.Get("/", func(w http.ResponseWriter, req *http.Request) {
			q := req.URL.Query()
			f := accounting.InvoiceFilter{}
			if v := q.Get("status"); v != "" {
				f.Status = v
			}
			...
			list, err := arSvc.List(req.Context(), f)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
		...
		r.Post("/", func(w http.ResponseWriter, req *http.Request) {
			...
			inv, err := arSvc.Create(req.Context(), in)
			if err != nil {
				http.Error(w, err.Error(), http.StatusBadRequest)
				return
			}
		...
		r.Post("/{id}/book", func(w http.ResponseWriter, req *http.Request) {
			...
			inv, err := arSvc.Book(req.Context(), id)
			if err != nil {
				http.Error(w, err.Error(), http.StatusBadRequest)
				return
			}
		...
		r.Get("/{id}", func(w http.ResponseWriter, req *http.Request) {
			...
			inv, err := arSvc.Get(req.Context(), id)
			if err != nil {
				http.Error(w, err.Error(), http.StatusNotFound)
				return
			}
		...
		r.Post("/{id}/payments", func(w http.ResponseWriter, req *http.Request) {
			idStr := chi.URLParam(req, "id")
			...
			_, err = paymentSvc.Apply(req.Context(), accounting.PaymentInput{
				InvoiceID: invoiceID,
				Amount:    in.Amount,
		...
		r.Get("/{id}/payments", func(w http.ResponseWriter, req *http.Request) {
			idStr := chi.URLParam(req, "id")
			invoiceID, err := uuid.Parse(idStr)
			if err != nil {
				http.Error(w, "Ungültige ID", http.StatusBadRequest)
			...
			list, err := paymentSvc.List(req.Context(), invoiceID)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
	...
	// Bankimport (vereinfachter manueller Import + optionaler Match)
	r.Post("/bank/statements", func(w http.ResponseWriter, req *http.Request) {
		var in accounting.BankStatementInput
		if err := json.NewDecoder(req.Body).Decode(&in); err != nil {
			http.Error(w, "Ungültige Eingabe", http.StatusBadRequest)
			return
		}
		id, err := bankSvc.Ingest(req.Context(), in)
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
	...
	r.Get("/bank/statements", func(w http.ResponseWriter, req *http.Request) {
		lim := 0
		...
		list, err := bankSvc.List(req.Context(), lim)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	r.Post("/bank/statements/{id}/match", func(w http.ResponseWriter, req *http.Request) {
		idStr := chi.URLParam(req, "id")
		stmtID, err := uuid.Parse(idStr)
		if err != nil {
			http.Error(w, "Ungültige ID", http.StatusBadRequest)
			return
		}
		...
		var invID *uuid.UUID
		if in.InvoiceID != nil {
			parsed, perr := uuid.Parse(*in.InvoiceID)
			if perr != nil {
				http.Error(w, "Ungültige InvoiceID", http.StatusBadRequest)
				return
			}
			invID = &parsed
		}
		payID, err := bankSvc.Match(req.Context(), stmtID, invID)
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
		writeJSON(w, http.StatusOK, map[string]any{"payment_id": payID})
	})

	// HR – Mitarbeitende
	r.Route("/hr/employees", func(r chi.Router) {
		r.Get("/", func(w http.ResponseWriter, req *http.Request) {
			lim := 0
			off := 0
			q := req.URL.Query()
			...
			list, err := empSvc.List(req.Context(), lim, off)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
		...
		r.Post("/", func(w http.ResponseWriter, req *http.Request) {
			...
			out, err := empSvc.Create(req.Context(), in)
			if err != nil {
				http.Error(w, err.Error(), http.StatusBadRequest)
				return
			}
		...
		r.Get("/{id}", func(w http.ResponseWriter, req *http.Request) {
			...
			emp, err := empSvc.Get(req.Context(), uid)
			if err != nil {
				http.Error(w, err.Error(), http.StatusNotFound)
				return
			}
		...
		r.Patch("/{id}", func(w http.ResponseWriter, req *http.Request) {
			...
			if err := empSvc.Update(req.Context(), uid, patch); err != nil {
				http.Error(w, err.Error(), http.StatusBadRequest)
				return
			}
			w.WriteHeader(http.StatusNoContent)
		...
		r.Get("/teams", func(w http.ResponseWriter, req *http.Request) {
			list, err := empSvc.ListTeams(req.Context())
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
	...
	// HR – Urlaubsanträge
	r.Route("/hr/leave-requests", func(r chi.Router) {
		r.Get("/", func(w http.ResponseWriter, req *http.Request) {
			var empID *uuid.UUID
			if v := req.URL.Query().Get("employee_id"); v != "" {
				if uid, err := uuid.Parse(v); err == nil {
					empID = &uid
				}
			}
			list, err := leaveSvc.List(req.Context(), empID, 50, 0)
			if err != nil {
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
			}
		...
		r.Post("/", func(w http.ResponseWriter, req *http.Request) {
			var in hr.LeaveRequest
			if err := json.NewDecoder(req.Body).Decode(&in); err != nil {
				http.Error(w, "Ungültige Eingabe", http.StatusBadRequest)
				return
			}
			out, err := leaveSvc.Create(req.Context(), in)
			if err != nil {
				http.Error(w, err.Error(), http.StatusBadRequest)
				return
			}
		...
		r.Post("/{id}/decision", func(w http.ResponseWriter, req *http.Request) {
			idStr := chi.URLParam(req, "id")
			uid, err := uuid.Parse(idStr)
			if err != nil {
				http.Error(w, "Ungültige ID", http.StatusBadRequest)
			...
			aid, err := uuid.Parse(in.ApproverID)
			if err != nil {
				http.Error(w, "Ungültige ApproverID", http.StatusBadRequest)
				return
			}
			if err := leaveSvc.Decide(req.Context(), uid, aid, in.Approve); err != nil {
				http.Error(w, err.Error(), http.StatusBadRequest)
				return
			}
			w.WriteHeader(http.StatusNoContent)
