...
func (s *Service) UploadMaterialDocument(ctx context.Context, materialID string, r io.Reader, filename, contentType string, companyID string) (*MaterialDocument, error) {
...
func (s *Service) ListMaterialDocuments(ctx context.Context, materialID string, companyID string) ([]MaterialDocument, error) {
	if _, err := s.Get(ctx, materialID, companyID); err != nil {
		return nil, err
	}
...
// OpenDocumentStream öffnet einen Download-Stream aus GridFS. Prüft VOR dem
// Öffnen des GridFS-Streams, dass die dokumentID tatsächlich mit einem
// material_documents-Eintrag verknüpft ist, dessen Material zum Mandanten
// companyID gehört (Backlog 0.22) - vorher konnte jede beliebige, bekannte
// oder erratene GridFS-ObjectID heruntergeladen werden, unabhängig vom
// Mandanten und sogar ohne jede Verknüpfung zu einem material_documents-
// Eintrag.
func (s *Service) OpenDocumentStream(ctx context.Context, documentID string, companyID string) (io.ReadCloser, string, string, int64, error) {
	if s.mg == nil || s.mongoDB == "" {
		return nil, "", "", 0, errors.New("MongoDB nicht konfiguriert")
	}
	if s.pg == nil {
		return nil, "", "", 0, errors.New("Postgres nicht konfiguriert")
	}
	...
	err = s.pg.QueryRow(ctx, `
        SELECT md.filename, md.content_type, md.length
          FROM material_documents md
          JOIN materials m ON m.id = md.material_id
         WHERE md.document_id = $1 AND m.company_id = $2
         LIMIT 1
    `, documentID, companyID).Scan(&filename, &contentType, &length)
	if err != nil {
		return nil, "", "", 0, errors.New("Dokument nicht gefunden")
	}
